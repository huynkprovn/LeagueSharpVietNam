...
namespace LeeSinSharp
{
    class LeeSin
    {
      ...
      public  static  Items.Item khienradun = new Items.Item(3143, 500);
        ...
        public static void combo()
        {
            castQFirstSmart();
            castQSecondSmart();
            if (IsBOTRK() && BladeOfRuinKing.Range <= 450 )
            {
                
                if (Player.Health <= Player.MaxHealth - LeeSinSharp.target.MaxHealth * 0.1)
                {
                    BladeOfRuinKing.Cast(LeeSinSharp.target);
                }
            }
            if (Radun() && khienradun.Range >= Player.Distance(LeeSinSharp.target))
            {
                khienradun.Cast();
                
            }
            if (IsHydra() && Hydra.Range >= Player.Distance(LeeSinSharp.target))
            {
                Hydra.Cast();

            }
            if (IsBilge() && BlidgeWater.Range >= Player.Distance(LeeSinSharp.target))
            {
                BlidgeWater.Cast(LeeSinSharp.target);
            }
            if (IsTiamat() && Tiamat.Range >= Player.Distance(LeeSinSharp.target))
            {
                Tiamat.Cast();
            }

            if (IsYoumuu() && Youmuu.Range >= Player.Distance(LeeSinSharp.target))
            {
                Youmuu.Cast();
            }
        ...
        public static bool getBackHarass()
        {
            ...
            Obj_AI_Base jumpOn = ObjectManager.Get<Obj_AI_Base>().Where(ally => ally.IsAlly && !(ally is Obj_AI_Turret) && !ally.IsMe && ally.Distance(Player.ServerPosition) < 700).OrderBy(tur => tur.Distance(closest_tower.ServerPosition)).First();
            W.Cast(jumpOn);
        ...
        public static bool IsIgnite()
        {
            if (LeeSinSharp.Config.Item("AutoIgnite").GetValue<bool>())
            {
        ...
        public static bool IsHydra()
        {
            if (LeeSinSharp.Config.Item("Automangxa").GetValue<bool>())
            {
        ...
        public static bool IsBOTRK()
        {
            if (LeeSinSharp.Config.Item("AutoBOTRK").GetValue<bool>())
            {
        ...
        public static bool IsYoumuu()
        {
            if (LeeSinSharp.Config.Item("AutoYoumuu").GetValue<bool>())
            {
        ...
        public static bool IsTiamat()
        {
            if (LeeSinSharp.Config.Item("AutoTiamat").GetValue<bool>())
            {
                return Tiamat.IsReady();
            }
            return false;
        }
        public static bool Radun()
        {
            if (LeeSinSharp.Config.Item("KhienRadun").GetValue<bool>())
            {
                return khienradun.IsReady();
            }
            return false;
        }
